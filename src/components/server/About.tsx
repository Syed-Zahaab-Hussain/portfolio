      ...
      <Reveal width="100%">
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <div className="flex flex-wrap justify-center gap-6">
             <button className="flex cursor-pointer items-center gap-2 px-8 py-3 rounded-lg bg-[#8b5cf6] text-white font-medium hover:bg-[#7c3aed] transition-all shadow-[0_0_20px_rgba(139,92,246,0.3)] hover:shadow-[0_0_30px_rgba(139,92,246,0.5)]">
                 <Download size={20} />
             ...
             {stats.map((stat, index) => (
                 <div key={index} className="group relative p-8 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-purple-500/30 transition-all duration-300 hover:-translate-y-1">
                    <div className="flex items-center justify-between mb-6">
                        <div className="p-4 rounded-full bg-white/5 text-gray-300 group-hover:text-purple-400 group-hover:bg-purple-500/10 transition-colors">
                            <stat.icon size={28} />
                        </div>
                        <span className="text-4xl font-bold text-white">{stat.value}</span>
                    </div>
                    <div>
                        <h3 className="text-sm font-bold text-gray-200 tracking-wider mb-2 uppercase">{stat.label}</h3>
                        <p className="text-xs text-gray-400">{stat.subLabel}</p>
                    </div>
                     <div className="absolute bottom-4 right-4 text-gray-600 group-hover:text-purple-400/70 transition-colors">
